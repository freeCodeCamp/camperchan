...
export const author: Command = {
  ...
  run: async (bot, interaction) => {
    try {
      ...
      const existsByUserId = await bot.db.authors.findUnique({
        where: {
          userId: member.id
        }
      });
      ...
      const existsByEmail = await bot.db.authors.findUnique({
        where: {
          email
        }
      });
